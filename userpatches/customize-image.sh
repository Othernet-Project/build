...
DreamcatcherFixes() {
        if [ "$BOARD" = "dreamcatcher" ]
        then
		# apply distro fixes: switch on audio, setup repo, remove armbian repo, enable getty on usb-serial, enable modules
		tar xvf /tmp/overlay/dreamcatcher-fixes.tgz -C /
		rm /etc/apt/sources.list.d/armbian.list
		for i in /etc/apt/sources.list.d/*
		do
			if [ "$i" = "/etc/apt/sources.list.d/othernet-${RELEASE}.list" ]
			then
				continue
			else
				rm "$i"
		...
		apt-key add /tmp/overlay/Release.gpg

		if [ "$BUILD_DESKTOP" = "yes" ]
		then
			tar xvf /tmp/overlay/dreamcatcher-fixes-desktop.tgz -C /
		fi
	fi
}
